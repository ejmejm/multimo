...
def kill_client(client, process_list):
    client.kill()
    if client in process_list:
        idx = process_list.index(client)
        process_list = process_list[:idx] + \
            process_list[idx+1:]

def cleanup_clients(process_list):
...
atexit.register(cleanup_clients, CLIENT_PROCESSES)
